...
/** The OCR-box projection math: un-rotating ML Kit boxes and mapping them onto the view. */
public class ScanGeometryTest {
    ...
    public void plainScalingMapsProportionally() {
        ...
        assertArrayEquals(new float[] {108, 168.75f, 216, 337.5f},
                ScanGeometry.imageBoxToView(new float[] {64, 100, 128, 200}, 640, 480, corners),
                0.01f);
    }

    @Test
    public void rotatedDisplayTransformStillYieldsAnUprightRect() {
        ...
        float[] mapped = ScanGeometry.imageBoxToView(
                new float[] {0, 0, 640, 480}, 640, 480, corners);
        // The whole image maps to the whole view, re-boxed the right way out.
        assertArrayEquals(new float[] {0, 0, 480, 640}, mapped, 0.01f);
    }

    @Test
    public void cropOffsetShiftsTheBox() {
        // The view shows a centre crop: image x is shifted left by 60 view px.
        float[] corners = {-60, 0, 1020, 0, -60, 810};
        float[] mapped = ScanGeometry.imageBoxToView(
                new float[] {0, 0, 640, 480}, 640, 480, corners);
        assertArrayEquals(new float[] {-60, 0, 1020, 810}, mapped, 0.01f);
    }
}
